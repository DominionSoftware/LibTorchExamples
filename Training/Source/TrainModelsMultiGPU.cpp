...
namespace torch_explorer
{
		...
		size_t logInterval)
	{
		try
		{
			...
			// Create mapping tensor on the correct device
			...
			std::cout << "Initial mapping tensor dtype: " << mapping_tensor.dtype() << std::endl;
			...
			for (size_t epoch = 0; epoch < num_epochs; ++epoch)
			{
				...
				for (auto& batch : *trainLoader)
				{
					...
					for (const auto& example : batch)
					{
						...
						// Convert target to Int64 immediately
					...
					if (batch_idx == 0)
					{
						...
						std::cout << "Target dtype: " << target.dtype() << std::endl;
					}
					...
					auto coarse_target = mapping_tensor.index_select(0, target_coarse);
...

				{
					...
					for (const auto& batch : *testLoader)
					{
						...
						{
							data_vec.push_back(example.data);
							// Convert target to Int64 in validation as well
							target_vec.push_back(example.target.to(torch::kInt64));
						}

						// Stack and move to devices
						auto data = stack(data_vec);
						auto target = stack(target_vec);
